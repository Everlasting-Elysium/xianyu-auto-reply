...
import {
  ...
  ChevronLeft, ChevronRight, Server, Link2, ShoppingCart,
} from 'lucide-react'
import {
...
type TabKey = 'configs' | 'mappings' | 'orders'

const tabs: Array<{ key: TabKey; label: string; icon: React.ElementType }> = [
  { key: 'configs', label: '平台账号', icon: Server },
  { key: 'mappings', label: '套餐映射', icon: Link2 },
...
export function ChargePlatforms() {
  ...
  if (loading && configs.length === 0 && mappings.length === 0 && orders.length === 0) {
    return <PageLoading />
  }
